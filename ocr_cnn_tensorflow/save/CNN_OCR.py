...
for i in range(epochs):
    ...
    test_acc = sess.run(accuracy, feed_dict={x: X_test[test_index[0:500]], y: y_test[test_index[0:500]], keep_prob: 1.0})
    train_acc = sess.run(accuracy, feed_dict={x: X_train[train_index[0:500]], y: y_train[train_index[0:500]], keep_prob: 1.0})
    test_loss = sess.run(loss,feed_dict={x: X_test[test_index[0:500]], y: y_test[test_index[0:500]], keep_prob: 1.0})
    train_loss = sess.run(loss, feed_dict={x: X_train[train_index[0:500]], y: y_train[train_index[0:500]], keep_prob: 1.0})
...
result = tf.argmax(y_conv, 2)
image.write('1567', './data/1567.png')
pre_img = cv2.imread('./data/1567.png')
sess.run(result,feed_dict={x:[pre_img],keep_prob: 1.0})
